    ...
    <div className="relative flex min-h-[calc(100vh-61px)] items-center justify-center overflow-hidden px-6 py-16">
      ...
      >
        <h1 className="font-display text-3xl font-bold tracking-tight">
          {isLogin ? (
            <>{t.auth.welcomePre}<span className="text-gradient">{t.auth.welcomeSpan}</span></>
          ) : (
            <>{t.auth.joinPre}<span className="text-gradient">{t.auth.joinSpan}</span></>
          )}
        </h1>
        ...
        <form onSubmit={submit} className="mt-8 flex flex-col gap-4">
          ...
          <input
            ...
            placeholder={isLogin ? t.auth.password : t.auth.passwordMin}
            required
